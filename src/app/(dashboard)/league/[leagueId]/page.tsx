          ...
          <CardContent className="space-y-2">
            {league.teams.map((team, i) => (
              <div key={team.id} className="flex items-center gap-3 py-2 border-b border-slate-800 last:border-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-slate-400 text-xs truncate">{team.user.name}</p>
                </div>
                {team.userId === userId && (
            ...
            {league.teams.length < league.maxTeams && (
              <div className="pt-2 text-center">
                <p className="text-slate-500 text-xs mb-1">{league.maxTeams - league.teams.length} spots remaining</p>
                <p className="text-slate-400 text-xs">Invite code: <span className="text-slate-200 font-mono">{league.inviteCode}</span></p>
              </div>
            )}
          </CardContent>
        </Card>
